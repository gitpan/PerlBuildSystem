...
, $config, $file_to_build, $dependencies, $triggering_dependencies, $tree) = @_ ;
...
{
my($shell_command, $shell_command_info, $tree) = @_ ;

RunPluginSubs('EvaluateShellCommand', \$shell_command, $tree) ;

my $config = $tree->{__CONFIG} ;
my $file_to_build = $tree->{__BUILD_NAME} ;

my @dependencies = map {$tree->{$_}{__BUILD_NAME} ;} grep { $_ !~ /^__/ ;}(keys %$tree) ;
