...
Special handling of repositories and include pathes is left to the user (thus this user modifiable plugin) as there
...
my @repository_pathes = PBS::Build::NodeBuilder::GetNodeRepositories($tree) ;
...
while($$shell_command_ref =~ /([^\s]+)?\%PBS_REPOSITORIES/g)
	...
	my $replacement = '';
	for my $repository_path (@repository_pathes)
		{
		if($evaluate_shell_command_verbose)
			{
